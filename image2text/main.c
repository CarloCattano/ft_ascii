 ...
 * Thus changing our buffer size to 5 chars for escape sequence and 1 char for the letter
 */
...
int main(int ac, char **av)
{
    ...
    int new_height = atoi(av[3]);

    unsigned char *new_img = malloc(new_width * new_height * channels);
    
    stbir_resize_uint8_linear(img, width, height, 0, new_img, new_width, new_height, 0, channels);
